...
- Organized 4,389 sales files into `year=YYYY/month=MM/store=SXX` partitions.
- Created the `sales-data` MinIO bucket and uploaded 4,457 objects.
...
### Task B: Master Data and Idempotent Warehouse Load - Completed
- Loaded `stores`, `product_categories`, `products`, and `price_revisions` into PostgreSQL.
- Built DuckDB `sales_fact` from MinIO data.
- Normalized mixed schemas and timestamps, then deduplicated by `(bill_no, line_no)`.
- Added automatic connection cleanup to prevent DuckDB file locks.
...
- Created `dim_store`, `dim_category`, and `dim_product` dimensions.
...
### Task E: Cross-System Query Execution - Implemented
- Reads sales CSV data from MinIO through DuckDB `httpfs`.
- Reads PostgreSQL master dimensions through DuckDB's `postgres` extension.
- Joins sales to products and categories, then aggregates revenue by category.

#### Execution tree

```text
EXPLAIN ANALYZE
└── HASH_GROUP_BY: category_name, SUM(quantity * unit_price)
    └── HASH_JOIN: sales.product_code = products.product_code
        ├── CSV_SCAN: s3://sales-data/partitioned_sales/...
...
- Compared DuckDB `fact_sales` revenue with signed-off `finance_monthly.csv` for all 12 months.
- Used the actual `revenue_inr` column and `YYYY-MM` month format.

#### Terminal output

```text
month | finance_revenue | pipeline_revenue | variance
2024-01 | 38446071.33 | 27247666.47 | -11198404.86
2024-02 | 34887085.55 | 24801384.00 | -10085701.55
2024-03 | 42457899.09 | 29694679.19 | -12763219.90
2024-04 | 37958457.37 | 27089951.51 | -10868505.86
2024-05 | 41764716.40 | 30000211.52 | -11764504.88
2024-06 | 38987082.82 | 27925693.86 | -11061388.96
2024-07 | 40527291.81 | 28741506.91 | -11785784.90
2024-08 | 45252181.75 | 31792176.03 | -13460005.72
2024-09 | 44615037.46 | 31671539.40 | -12943498.06
2024-10 | 56359195.92 | 40285483.40 | -16073712.52
2024-11 | 51583838.47 | 36956115.22 | -14627723.25
2024-12 | 50745209.00 | 36306190.73 | -14439018.27
```
